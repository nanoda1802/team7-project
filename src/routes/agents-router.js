      ...
      if (!myAgent || myAgent.count < count) {
        ...
        const update = await prisma.users.update({
          ...
          data: {
            ...
            myAgent: {
              update: {
                where: { userKey_agentKey: { agentKey: myAgent.agentKey, userKey: myAgent.userKey } },
                data: {
...
router.patch("/users/agents/gacha", authMiddleware, champVerification, async (req, res, next) => {
  try {
    ...
    // 픽업 챔피언 확인
    if (agent.grade !== "s") return res.status(400).json({ errorMessage: "<pickup> 챔피언은 S급이여야 합니다!" });
    //할인 적용
    if (count >= 10) {
      totalCost = count * 900;
    ...
    if (userAssets.cash < totalCost) return res.status(400).json({ errorMessage: "캐시가 부족합니다." });
    ...
    if (!agents) return res.status(404).json({ errorMessage: "챔피언 조회에 실패 하였습니다." });
    // 급 나누기
...
router.patch("/users/agents/intensify", authMiddleware, champVerification, async (req, res) => {
  try {
    ...
    if (!myAgent) return res.status(404).json({ errorMessage: "현재 보유한 챔피언이 아닙니다" });
    ...
    if (currentLevel >= 15) return res.status(401).json({ message: "이미 최대 강화에 도달한 챔피언 입니다!" });

    // 보유 재료 확인
    const asset = await prisma.assets.findFirst({ where: { userKey: user.userKey } });
    let balanceEnhancer = asset.enhancer - requiredEnhancer;

    // 강화 재료가 부족한 경우, 마일리지 100 당 강화 재료 1개로 인식하여 부족분 충족 확인
    if (balanceEnhancer < 0) {
      // 그래도 모자라면 각각 얼마나 모자른지 응답
...
router.patch("/users/agents/promote", authMiddleware, champVerification, async (req, res) => {
  try {
    ...
    // 보유 선수 확인
    const myAgent = await prisma.myAgents.findFirst({ where: { agentKey: agent.agentKey, userKey: user.userKey } });
    if (!myAgent) return res.status(404).json({ errorMessage: "현재 보유하고 있는 챔피언이 아닙니다." });
    // 최대 승급된 챔피언 시 거부
    if (myAgent.class >= 6) return res.status(400).json({ errorMessage: "이미 최대 승급을 달성한 선수입니다" });
    // 중복 보유량이 없을 경우 거부
    if (myAgent.count <= 1) {
