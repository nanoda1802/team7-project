...
router.put("/users/:key/formation", strVerification, async (req, res, next) => {
  ...
  for (let i = 0; i < formation.length; i++) {
    ...
    //보유 챔피언 확인
    if (!myAgent[i]) return res.status(400).json({ errorMessage: `${formation[i]}(은)는 현재 보유한 챔피언이 아닙니다.` });
    ...
    [
      ...new Set(
        agent
          .map((e) => e.team)
          .filter((e, idx, arr) => {
            if (idx !== arr.indexOf(e) && arr.indexOf(e) !== -1) return true;
            else return false;
          }),
      ),
    ].join("") || "none";
  ...
  return res.status(201).json({
    ...
    squd: [`${myAgent[0].name}(${agent[0].position})`, `${myAgent[1].name}(${agent[1].position})`, `${myAgent[2].name}(${agent[2].position})`],
    synergy,
  });
});
...
router.patch("/users/:key/favorite", strVerification, async (req, res, next) => {
  ...
  // 보유 챔피언 확인
  if (!myAgent) return res.status(400).json({ errorMessage: `${agent}(은)는 현재 보유한 챔피언이 아닙니다.` });
