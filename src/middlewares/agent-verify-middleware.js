    ...
    if (Array.isArray(agentValues?.formation) || Array.isArray(agentValues)) {
      ...
      if (agentValues?.formation) {
        for (let agentKey of agentValues.formation) {
          if (isNaN(+agentKey))
            return res.status(400).json({ errorMessage: "선택할 챔피언의 <agent_key>를 숫자로 입력해주세요" });
          // 챔피언 존재 여부 확인
      ...
      // 입력값 확인
      if (!agentKey || isNaN(+agentKey))
        return res.status(400).json({ errorMessage: "선택할 챔피언의 <agent_key>를 숫자로 입력해주세요" });

      const agent = await prisma.agents.findFirst({ where: { agentKey } });
      if (!agent)
        return res.status(404).json({ errorMessage: `<agent_key> ${agentKey}에 해당하는 챔피언은 존재하지 않습니다` });
      // 챔프 값 반환
      req.agent = agent;
    }
    next();
